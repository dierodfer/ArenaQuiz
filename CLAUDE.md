...
- Los participantes acceden a la pregunta y a las estadísticas vía **funciones RPC** (`security definer`, bypasean RLS de forma controlada):
  ...
  - `submit_answer(p_question_id, p_participant_id, p_answer)`: registra o **actualiza** la respuesta y ajusta el score en el servidor de forma atómica (el cliente nunca decide acierto ni score). Primera respuesta → INSERT + (+100 si acierta). Si ya existe y la sala sigue `in_question` y la opción cambia → UPDATE de la fila, recalcula `is_correct` y ajusta el score por el delta (+100 si la nueva acierta, -100 si la anterior acertaba); esto permite **cambiar la respuesta mientras corre el tiempo**. Si la sala ya no está `in_question` (resultados) o se reenvía la misma opción → idempotente: devuelve lo almacenado sin tocar el score (soporta reconexión / doble clic).
