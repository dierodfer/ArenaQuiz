...
- Solo el timer del admin cambia el `status` (a `showing_results` al llegar a 0). El timer del participante no toca la sala, pero sí dispara el **envío de su respuesta** al llegar a 0 (ver siguiente punto) y deshabilita las opciones.
- **La respuesta del participante se elige en local y se sube una sola vez, al final** (`ParticipantRoom`). Mientras corre el timer, tocar una opción solo actualiza el estado local (`selectedRef`/`myAnswer`), sin llamar a Supabase, así que se puede **cambiar libremente** cuantas veces se quiera. La RPC `submit_answer` se invoca una única vez (`submitFinal`, protegida por `submittedRef`) cuando el timer del participante llega a 0; un `useEffect` sobre `showing_results` la vuelve a llamar como red de seguridad (idempotente) por si el reloj local no llegó a 0 o el admin cerró antes. Así se hace **1 sola llamada a BBDD por pregunta** en vez de una por cada cambio, y el marcador/corrección solo se aplican al enviar (cuando ya no se puede cambiar), sin revelar la correcta en vivo. Sin cambios en `schema.sql`: `submit_answer` sigue siendo idempotente, no actualiza respuestas existentes.
