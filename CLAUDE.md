...
- `participants`: FK a room, `username` (3-10 chars, validado en cliente con `validateUsername`), `email` (opcional, máx. 50 chars, `validateEmail`), `score`. Límite de **200 participantes por sala** (`MAX_PARTICIPANTS`), validado por RLS en el INSERT y por el cliente antes de insertar. El admin dueño puede expulsar participantes en el lobby (botón "Editar" en `AdminRoom`, estado `open`); el DELETE se propaga por realtime (`replica identity full` para que el filtro por `room_id` reciba el evento).
...
- Los participantes acceden a la pregunta y a las estadísticas vía **funciones RPC** (`security definer`, bypasean RLS de forma controlada):
  ...
  - `submit_answer(p_question_id, p_participant_id, p_answer)`: registra o **actualiza** la respuesta y ajusta el score en el servidor de forma atómica (el cliente nunca decide acierto ni score). Primera respuesta → INSERT + (+1 si acierta). Si ya existe y la sala sigue `in_question` y la opción cambia → UPDATE de la fila, recalcula `is_correct` y ajusta el score por el delta (+1 si la nueva acierta, -1 si la anterior acertaba); esto permite **cambiar la respuesta mientras corre el tiempo**. Si la sala ya no está `in_question` (resultados) o se reenvía la misma opción → idempotente: devuelve lo almacenado sin tocar el score (soporta reconexión / doble clic).
  - `get_my_result(p_question_id, p_participant_id)`: devuelve en **una sola llamada** `{ answer, is_correct, score, total, correct }` del participante (`answer`/`is_correct` `null` si no respondió). Sustituye a las antiguas `get_my_answer` + `get_question_stats` + un `select` de score: el participante hacía 3 llamadas al entrar en resultados y ahora hace 1, lo que reduce la ráfaga de peticiones cuando ~150 clientes reaccionan a la vez a `showing_results` (la lectura se difiere además con un jitter aleatorio, `RESULT_FETCH_JITTER_MS`). Usado para reconciliar el estado autoritativo tras resultados / reconexión.
- `useCurrentQuestion` (compartido por admin y participante) llama siempre a estas RPC, no a las tablas directamente. El admin sí lee `questions`/`room_questions`/`answers` directamente en `QuestionBank`, `CreateRoom` y el tally en vivo de `AdminRoom` (permitido por RLS al ser el dueño).
...
- `submit_answer` permite cambiar la respuesta mientras la sala está `in_question` (UPDATE con ajuste de score por delta) y se bloquea en `showing_results`; reenviar la misma opción es idempotente (sin doble score).
...
- Solo el timer del admin cambia el `status` (a `showing_results` al llegar a 0). El timer del participante es puramente visual; al llegar a 0 solo deshabilita las opciones y espera el evento.
- **El participante puede cambiar su respuesta mientras corre el tiempo** (`ParticipantRoom`). Cada toque en una opción llama a `submit_answer`: la **primera** vez hace INSERT (el admin la ve en vivo vía su realtime de `answers`) y los **siguientes** cambios hacen UPDATE de la misma fila mientras la sala siga `in_question`, recalculando `is_correct` y ajustando el score por el delta (+1/-1) en el servidor. Los UPDATE no inflan el contador del admin (solo escucha INSERT), así que el conteo en vivo sigue siendo correcto. El **marcador y la corrección NO se aplican en vivo** en el cliente (durante `in_question` solo se muestra el anillo de la opción marcada): al pasar a `showing_results` se lee el estado final autoritativo con `get_my_answer` (acierto) y un `select` de `participants.score`, para no revelar la correcta mientras aún se puede cambiar y ser inmune a carreras si el último cambio iba en vuelo. En `showing_results` la respuesta queda bloqueada (la RPC devuelve lo almacenado sin tocar score).
