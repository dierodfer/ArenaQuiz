...
- En la vista de monitoreo del admin, `AnswerBreakdown` muestra cada respuesta posible (texto de la opción) y, justo debajo, el % y el número de respuestas; se conserva al pasar de `in_question` a `showing_results` (los `liveAnswers` solo se limpian al cambiar de pregunta).
